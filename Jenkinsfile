...
pipeline {
    ...
    environment {
        GIT_URL = "https://github.com/LCA-PJT2/test-service.git"
            ...
            steps {
                script {
                    withCredentials([string(credentialsId: 'github-pat', variable: 'GITHUB_TOKEN')]) {
                        ...
                        # 2. image 태그 업데이트
                        sed -i 's|image: ${DOCKER_REGISTRY}/${APP_NAME}:.*|image: ${DOCKER_REGISTRY}/${APP_NAME}:${APP_VERSION}|' test-service/prd/test-service-deploy.yml

                        # 3. Git 설정 및 커밋
                        git config user.name "minju26"
                        git config user.email "mjalswn26@gmail.com"
                        git add test-service/prd/test-service-deploy.yml
                        git commit -m "chore(test): 버전 업데이트 ${APP_VERSION}"
